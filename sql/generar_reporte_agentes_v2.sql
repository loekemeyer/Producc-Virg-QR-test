-- v2 post-fix error_envio 2026-07-03 (migración fix_error_envio_antijoin_y_recuperados)
-- Cambios vs original (backup en generar_reporte_agentes_original.sql):
--   3) error_envio: excluye falsos positivos (client_id que SÍ existe en Registros_Produccion_Virgilio)
--      y deduplica por client_id (página y SW auditan el mismo ítem repetido).
--   3b) NUEVA categoría envio_recuperado (severidad baja): intentos cuyo client_id SÍ llegó,
--      agrupados por legajo+opcion+día con client_id distintos. El cliente la renderiza desde v5.22.
-- El resto de las categorías: sin cambios.
...
begin
  ...
  -- 3) Envíos que FALLARON (v2: excluye los que SÍ llegaron a Registros por client_id
  --    y deduplica por client_id — página y SW auditan el mismo ítem repetido)
  insert into public.reporte_agentes (categoria, severidad, titulo, detalle, valor, ts_evento)
  with intentos_fallidos as (
    select distinct on (coalesce(a.client_id, a.id::text))
  ...
  -- 3b) Envíos RECUPERADOS: fallaron pero el reintento los subió igual (client_id ya en Registros).
  --     Informativo — no se perdió nada; sirve para detectar agujeros de wifi.
  insert into public.reporte_agentes (categoria, severidad, titulo, detalle, valor, ts_evento)
  select 'envio_recuperado', 'baja',
         'Envío con reintento — ' || coalesce(nullif(trim(a.opcion), ''), '?'),
         count(distinct a.client_id) || ' ítem(s) · legajo ' || coalesce(nullif(trim(a.legajo), ''), '?')
           || ' · ' || to_char((coalesce(a.ts, a.ts_cliente) at time zone 'America/Argentina/Buenos_Aires')::date, 'DD/MM'),
  ...
  group by coalesce(nullif(trim(a.opcion), ''), '?'), coalesce(nullif(trim(a.legajo), ''), '?'),
           (coalesce(a.ts, a.ts_cliente) at time zone 'America/Argentina/Buenos_Aires')::date
  order by max(coalesce(a.ts, a.ts_cliente)) desc limit 20;
